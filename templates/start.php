		...
		<main class="feature">
			...
			<section>
				...
				<?php if(!$Posts->is_empty()){
					$Posts->each(function($post) use ($server){ include 'components/preview-post.php'; });
				} else { ?>
			...
			<section class="events image" style="--bg: url('<?= $server->url ?>/resources/images/Kalender.jpg');">
				<h2>Termine</h2>
				<?php if(!$Events->is_empty()){
					$Events->each(function($event) use ($server){ include 'components/event.php'; });
				} else { ?>
