		...
		<main>
			<article class="motion">
				<header>
					...
					<p class="status" data-status="<?= $Motion->status->value ?>">
						<?= match($Motion->status->value){
							'draft' => 'Entwurf zur Abstimmung',
							'accepted' => 'Angenommen',
							'rejected' => 'Abgelehnt'
						} ?>
						· Bezirksratssitzung am
						<time datetime="<?= $Motion->timestamp?->to_iso8601() ?>">
							<?= $Motion->timestamp?->format('d. MMMM Y') ?>
						</time>
					</p>
				</header>
				<section>
					...
					<a class="file" href="<?= $Motion->document->src() ?>">
						<h3 class="title"><?= $Motion->document->name ?></h3>
